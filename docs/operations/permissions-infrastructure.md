...
| `cloudwatch` | Target-tracking policies create the alarms that drive scale in/out; the optional `cardinal-satellite-cwmetrics` stack creates a metric stream. | `PutMetricAlarm`, `DeleteAlarms`, `DescribeAlarms`, `PutMetricStream`, `DeleteMetricStream`, `GetMetricStream`, `ListMetricStreams`, `TagResource`, `UntagResource`, `ListTagsForResource` |
| `firehose` | Delivery stream that writes the metric stream into the raw bucket (`cardinal-satellite-cwmetrics` only). | `CreateDeliveryStream`, `DeleteDeliveryStream`, `DescribeDeliveryStream`, `UpdateDestination`, `ListDeliveryStreams`, `TagDeliveryStream`, `UntagDeliveryStream`, `ListTagsForDeliveryStream` |
