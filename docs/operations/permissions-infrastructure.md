...
| `iam` | `iam:PassRole` only; pass the customer-supplied `TaskRoleArn` and `ExecutionRoleArn` to ECS, and (optionally) `CertLambdaRoleArn` to Lambda on the PEM cert path. No `iam:CreateRole` or other write actions. | `PassRole` |
...
| `lambda` | Optional cert-import Lambda only -- created when `CertificateArn` is empty (PEM-import path). Not needed if you supply an ACM cert ARN; environments that cannot run Lambda must do so. | `CreateFunction`, `DeleteFunction`, `UpdateFunctionCode`, `UpdateFunctionConfiguration`, `GetFunction`, `GetFunctionConfiguration`, `InvokeFunction`, `TagResource`, `UntagResource`, `ListTags` |
