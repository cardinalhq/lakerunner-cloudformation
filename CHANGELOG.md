...
- **IAM:** the shared `ExecutionRole` (generated by `lakerunner_infra_base.py`,
  ...
  via a new `ResolveCardinalSsm` statement. This grant is required: without it
  ECS cannot resolve the `MAESTRO_SATELLITE_CONFIG` SSM Secret and maestro fails
  to start (`ResourceInitializationError` / `AccessDeniedException`). The
  generated role now includes it — redeploy the infra-base stack so the role
  updates before deploying the services stack. Operators who supply their own
  execution role MUST add this grant themselves.
