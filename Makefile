...
test:		## Run unit tests (working tests only)
	source $(VENV_DIR)/bin/activate && $(PYTEST) tests/test_common_infra.py tests/test_*_simple.py tests/test_demo_otel_collector.py tests/test_mcp_combined.py tests/test_parameter_validation.py tests/test_condition_validation.py -v
...
test-mcp-combined:	## Run tests for MCP Combined Service template
	source $(VENV_DIR)/bin/activate && $(PYTEST) tests/test_mcp_combined.py -v
