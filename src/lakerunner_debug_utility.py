...
t = Template()
t.set_description("Lakerunner utility task runner (single template): defines TaskDefinition and runs it via Custom Resource.")
...
CommandArguments = t.add_parameter(Parameter(
    ...
    Description="REQUIRED: Command arguments to pass to lakerunner binary."
))
...
t.set_metadata({
    "AWS::CloudFormation::Interface": {
        ...
        "ParameterLabels": {
            ...
            "CommandArguments": {"default": "Command Arguments"},
...
# -----------------------
# Task Role (runtime permissions for the utility task)
# -----------------------
...
# -----------------------
RunUtility = t.add_resource(RunEcsTask(
    "RunUtility",
...
t.add_output(Output(
    "TaskDefinitionArn",
    Value=GetAtt(TaskDef, "TaskDefinitionArn"),
...
t.add_output(Output(
    ...
    Export=Export(name=Sub("${AWS::StackName}-RunUtilityId"))
))
