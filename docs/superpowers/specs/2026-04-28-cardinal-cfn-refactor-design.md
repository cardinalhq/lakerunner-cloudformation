...
- `cluster.yaml` (ECS cluster + Cloud Map namespace + base log group) is **deleted as a nested child**. Neither the cluster nor the namespace lives in CloudFormation any more. The customer pre-creates both out of band and passes the identifiers (`CLUSTER_NAME`, `CLUSTER_ARN`, `SERVICE_NAMESPACE_ID`, `SERVICE_NAMESPACE_NAME`) into `data-setup.sh` as env vars; the script forwards them verbatim into its JSON output. The base log group was unused and is gone entirely.
...
- The lakerunner stack creates **no security groups, no IAM roles, no compute-plane resources**. The customer (or their IT) supplies all of them as ARN/ID parameters: `TaskRoleArn`, `ExecutionRoleArn`, `MigrationLambdaRoleArn`, `CertLambdaRoleArn`, `TaskSgId`, `AlbSgId`, plus the cluster + namespace identifiers above. There is one shared task role across every ECS service; per-service task roles were collapsed to keep the parameter surface small.
- Single-install assumption: the data-layer resource names the script creates (`cardinal-db`, `cardinal-ingest-*`, `/cardinal/*` SSM, `cardinal-*` secrets, `cardinal-ingest` SQS) are fixed. One install per AWS account/region. Customers running multiple installs use separate accounts.
