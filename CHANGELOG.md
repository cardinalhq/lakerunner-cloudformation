...
`MaestroImage` v1.93.1 → v1.93.3 (both digest-pinned).
...
**Image bump:** lakerunner `v1.67.1` → `v1.68.0`, maestro `v1.72.1` → `v1.73.1`.
Upgrade action: redeploy the root stack. The lakerunner bump retriggers the DB
migrator (reruns once, idempotent) before the service tiers update.
...
- **Removed:** `SATELLITE_CONFIG` / `SATELLITE_CONFIG_FILE` / `CENTRAL_COLLECTOR_NAME` /
  `SATELLITES_PARAM_NAME` driver env vars and the corresponding SSM compose+write step.
  The driver no longer synthesizes `/cardinal/satellites` or writes any SSM parameter.
- **Removed stack parameter:** `SatellitesParamName` (from root and maestro child).
- **Restored stack parameters:** `QueueUrl` and `QueueRoleArn` (root + services-process child);
  `QueueUrl` is sourced from `RawQueueUrl` and `QueueRoleArn` from `LakerunnerAccessRoleArn`
  on the satellite-infra-base stack, same as prior to v1.5.0.
- **Restored:** `MAESTRO_BOOTSTRAP_BUCKET_NAME/REGION/CLOUD_PROVIDER/COLLECTOR_NAME` env vars
  on the maestro container (and the `BucketName` parameter it reads from); Maestro provisions
  the single output bucket storage line via these on first start.
- **Restored:** `SQS_QUEUE_URL`, `SQS_REGION`, `SQS_ROLE_ARN` env vars on the `pubsub-sqs`
  container (from `QueueUrl`/`QueueRoleArn` parameters).
- **IAM:** `ResolveCardinalSsm` statement removed from the `ExecutionRole`
  (generated by `lakerunner-infra-base`); the SSM grant is no longer needed.
  Redeploy infra-base to drop the statement from the role. Operators who maintain
  a custom execution role may remove the `/cardinal/*` SSM grant.
...
- **Upgrade action:** remove `SATELLITE_CONFIG*` env vars from deploy invocations.
  ...
  Redeploy infra-base so the `ExecutionRole` loses the now-unnecessary SSM grant.
...
- **IAM:** the shared `ExecutionRole` (generated by `lakerunner_infra_base.py`,
  threaded into every task definition including maestro) now grants
  `ssm:GetParameters` / `ssm:GetParameter` on
  `arn:${Partition}:ssm:${Region}:${Account}:parameter/cardinal/*` (the install's
  own SSM namespace) via a new `ResolveCardinalSsm` statement. This grant is
  required: without it ECS cannot resolve the `MAESTRO_SATELLITE_CONFIG` SSM
  Secret and maestro fails to start (`ResourceInitializationError` /
  `AccessDeniedException`). The generated role now includes it — redeploy the
  infra-base stack so the role updates before deploying the services stack. A
  `SATELLITES_PARAM_NAME` override is covered as long as it stays under
  `/cardinal/`. Operators who supply their own execution role MUST add this grant
  themselves.
