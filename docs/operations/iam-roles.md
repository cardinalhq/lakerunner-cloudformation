...
task definition (including maestro) as `ExecutionRoleArn`. It carried an
additional `ResolveCardinalSsm` statement until v1.6.0; that grant is no longer
needed and operators maintaining a custom execution role may drop it.
...
- `maestro`: web UI + REST API. Talks to lrdb, calls lakerunner via
  ...
  Provisions the org and its output-bucket storage line through lakerunner's
  admin API; additional buckets are mapped in its superadmin UI.
