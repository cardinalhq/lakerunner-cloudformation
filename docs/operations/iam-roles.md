...
ResolveCardinalSsm:      ssm:GetParameter,GetParameters
                         on arn:${Partition}:ssm:<region>:<acct>:parameter/cardinal/satellites
...
`AccessDeniedException`. Operators who supply their own execution role MUST add
this grant.
...
the `cardinal-*` Secrets Manager name pattern. The SSM grant is scoped to the
exact `/cardinal/satellites` path.
